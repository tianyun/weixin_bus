...
public class BusApi extends HttpServlet {
	...
	protected static final Logger LOGGER = Logger.getLogger(BusApi.class);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if ("text".equals(methodType)) {
				...
				Map<String, String> lineInfoMap = BusAction.getBusWx(Content);
				if ("0000".equals(lineInfoMap.get("code"))) {
					resString = lineInfoMap.get("info");
					memCachedUtil.setMemCached(FromUserName, Content, new Date(System.currentTimeMillis() + 300000L));
				} else {
					try {
						Map<String, String> resMap = new HashMap();
						resMap = BusGetPlace.getPlaceInfo(Content);
						if ((resMap != null) && ("0000".equals(resMap.get("code")))) {
							if ("2".equals(resMap.get("size"))) {
								Map<String, String> stationMap1 = BusGetPlace
										.getPlace((String) resMap.get("stationid1"));
								Map<String, String> stationMap0 = BusGetPlace
										.getPlace((String) resMap.get("stationid0"));

								resString = (String) stationMap1.get("msg") + (String) stationMap0.get("msg");
							} else {
								Map<String, String> stationMap = BusGetPlace.getPlace((String) resMap.get("stationid"));
								System.out.println("stationMap" + stationMap.toString());
								resString = (String) resMap.get("stationName");
							}
						} else {
							resString = (String) resMap.get("msg");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
